...
项目目录结构导出工具。支持 GUI 模式和命令行模式，通过递归遍历目录树并生成带文件类型图标的目录结构文本，支持自定义忽略规则，可输出到 GUI 及控制台，或保存为文本文件。

**特点：**
- 内置丰富的文件类型图标映射，目录结构一目了然
- 支持自定义忽略规则（每行一条，支持通配符）
- 支持命令行模式和 GUI 模式
- 支持文件监控功能，文件变动时自动重新生成
- 可选保存到 `result/` 目录或直接输出到 GUI 及控制台

**文件监控功能**灵感来源于 [auto_summary](https://github.com/technogleb/auto_summary)
